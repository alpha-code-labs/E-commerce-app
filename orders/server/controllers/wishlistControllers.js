...
export const getWishlistsByUserId = async (req, res, next) => {
    try {
      ...
      // Find all wishlists linked to the user ID
      const wishlists = await Wishlist.find({ userId });
  
      if (wishlists.length === 0) {
        return res.status(404).json({ error: 'No wishlists found for the user' });
      }
...
// Get all wishlists created by a user
export const getWishlistItems = async (req, res, next) => {
    try {
      ...
      // Find all wishlists created by the user
      const wishlists = await Wishlist.find({ userId });
      ...
      // Extract and combine all wishlist items from each wishlist
      const wishlistItems = wishlists.flatMap((wishlist) => wishlist.items);
