    ...
    return(
        <>
             <div className="mx-auto w-[300px] h-[400px] max-w-[550px] bg-white text-left relative">
               ...
               {groups && (
  <>
    <p className="m-4">We have Identified these Groups. <br/> Are these correct?</p>
    <hr />
    <div className="w-full p-4 space-y-2">
      {groups.map((group, index) => (
        <div key={group.id || index} className="flex gap-5 w-full text-left">
          <p className="text-base w-2/3">{camelCaseToTitle(group)}</p>
          <input className="w-[20px] h-[20px]" defaultChecked={true} type="checkbox" />
        </div>
      ))}
    </div>
  </>
